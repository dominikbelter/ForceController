...
int CDXL_hal::dxl_hal_set_baud( float baudrate )
{
	struct serial_struct serinfo;
...
void CDXL_hal::dxl_hal_clear(void)
{
	tcflush(gSocket_fd, TCIFLUSH);
}

int CDXL_hal::dxl_hal_tx( unsigned char *pPacket, int numPacket )
{
	return write(gSocket_fd, pPacket, numPacket);
}

int CDXL_hal::dxl_hal_rx( unsigned char *pPacket, int numPacket )
{
