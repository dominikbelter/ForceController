...
int CDynamixel::dxl_initialize(int deviceIndex, int baudnum )
{
	float baudrate;
	baudrate = 2000000.0f / (float)(baudnum + 1);

	if( dxl_hal.dxl_hal_open(deviceIndex, baudrate) == 0 )
		return 0;
...
void CDynamixel::dxl_terminate(void)
{
	dxl_hal.dxl_hal_close();
}

void CDynamixel::dxl_tx_packet(void)
{
	unsigned char i;
	...
	if( gbCommStatus == COMM_RXTIMEOUT || gbCommStatus == COMM_RXCORRUPT )
		dxl_hal.dxl_hal_clear();

	TxNumByte = gbInstructionPacket[LENGTH] + 4;
	RealTxNumByte = dxl_hal.dxl_hal_tx( (unsigned char*)gbInstructionPacket, TxNumByte );
	...
	if( gbInstructionPacket[INSTRUCTION] == INST_READ )
		dxl_hal.dxl_hal_set_timeout( gbInstructionPacket[PARAMETER+1] + 6 );
	else
		dxl_hal.dxl_hal_set_timeout( 6 );

	gbCommStatus = COMM_TXSUCCESS;
}

void CDynamixel::dxl_rx_packet(void)
{
	...
	nRead = dxl_hal.dxl_hal_rx( (unsigned char*)&gbStatusPacket[gbRxGetLength], gbRxPacketLength - gbRxGetLength );
	gbRxGetLength += nRead;
	if( gbRxGetLength < gbRxPacketLength )
	{
		if( dxl_hal.dxl_hal_timeout() == 1 )
		{
			if(gbRxGetLength == 0)
...
void CDynamixel::dxl_txrx_packet(void)
{
	dxl_tx_packet();
...
int CDynamixel::dxl_get_result(void)
{
	return gbCommStatus;
}
...
int CDynamixel::dxl_get_rxpacket_length(void)
{
